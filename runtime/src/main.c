 ...
 * This is required because v1.3.x does not expose a public session-iteration API.
 */
...
static const char *get_session_state_string(UA_Session *session) {
    ...
    /* A session is "Closing" if its validTill is in the past (expired) but
     * still in the list. In open62541 v1.3, sessions are removed once fully
     * closed, so active sessions in the list are either Created or Activated. */
    return "Activated";
}
...
static void write_status_file(UA_Server *server) {
    ...
    LIST_FOREACH(sentry, &server->sessions, pointers) {
         ...
         * This is set when the secure channel is opened and is always valid. */
        char time_buf[64] = {0};
